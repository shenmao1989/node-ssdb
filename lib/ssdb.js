...
Client.prototype._registerEvents = function () {
  var self = this;
  this.conn.on('status_ok', function (cmd, data) {
    self.emit('status_ok', cmd, data)
  });
  this.conn.on('status_not_found', function (cmd) {
    self.emit('status_not_found', cmd)
  });
  this.conn.on('status_client_error', function (cmd) {
    self.emit('status_client_error', cmd)
  });
  this.conn.on('status_not_ok', function (status, cmd) {
    self.emit('status_not_ok', status, cmd)
  });
}
