...
Connection.prototype.onrecv = function(buf) {
  ...
  responses.forEach(function(response){
    ...
    switch (status) {
      ...
      default:
        self.emit('status_not_ok', command);
...
Client.prototype._registerEvents = function() {
  ...
  this.conn.on('status_not_found', function(cmd, data){self.emit('status_not_found', cmd)});
  this.conn.on('status_client_error', function(cmd, data){self.emit('status_client_error', cmd)});
  this.conn.on('status_not_ok', function(cmd, data){self.emit('status_not_ok', cmd)});
}
